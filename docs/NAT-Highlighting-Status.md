...
## 사용자 선택과 변경
사용자는 통합 유지 방식을 선택했습니다. 기본 파일명과 설치 대상은 PNET-Cisco-Dark-V3.ini 그대로이며 V2도 동기화했습니다. 전체 행 수는 432개입니다. 선택형 PNET-Cisco-NAT-V3.ini는 비교용으로 남아 있고 선택/설치를 요구하지 않습니다.
...
## 우선순위 보호
기존 SHOW_ACCESS_LISTS 블록은 내용 그대로 맨 앞으로 이동했습니다. 다음 NAT_CONTEXT_GUARDS 블록에서 host/network/to/is/from/via/neighbor/Originator:/list: 뒤와 쉼표 뒤 IPv4를 골드로 먼저 강조합니다. 공백도 매칭 범위에 포함될 수 있지만 라벨 자체는 칠하지 않습니다. 그 다음 NAT, BGP, ASA 전용 블록, 범용 상태 블록 순으로 이어집니다.
...
2026-09-15 `asa/ASA-SecureCRT-v1-Core.ini`와 `asa/ASA-SecureCRT-v1-Extended.ini`를 별도 목록으로 검증했습니다. 이후 Core는 46개, Extended는 64개 규칙으로 확장했으며 HA 인터페이스·동기화, ASAv 라이선스, SLA/AAA, 예약 reload·물리 환경과 Extended의 service-policy/ASP/resource/OSPF/인증서 등록 상태를 분리했습니다. 추가 사용자 화면에서 `Sync Done`, `Active Ifc Failure`의 Active, `Standby Ready Ifc Failure`의 Standby Ready, 경고형·상태형 ASAv `Unlicensed`, `No active entitlement: ...`는 의도한 색상으로 확인했습니다. Failure Reason의 `Ifc Failure`와 `inside: Failed`는 기본색으로 유지됐습니다.

사용자 요청으로 Extended의 default-color fallback을 제외한 63개 ASA 정규식을 통합 V2/V3의 `ASA_FIREWALL_OPERATIONAL_STATES` 블록으로 병합했습니다. 이 블록은 `BGP_SHOW_IP`와 범용 오류/정상 상태 블록 사이에 있어 기존 ACL·NAT 우선순위를 건드리지 않습니다.

병합한 `PNET-Cisco-Dark-V3`을 설치·선택한 2026-09-15 사용자 SecureCRT 화면에서는 다음을 확인했습니다.

- `show failover state`: Primary/Secondary 하늘색, Active 초록, Standby Ready 보라, Sync Done 초록. 과거 사유인 `Ifc Failure`와 `inside: Failed`는 기본색으로 유지.
- `show version | include entitlement` 및 `show version | include Unlicensed`: entitlement 부재와 Unlicensed 상태 빨강.
- `show nat`: Manual/Auto NAT Policies 구획과 `translate_hits`/`untranslate_hits` 하늘색.
- `show xlate`: 두 `NAT from` 행 하늘색.
- `show access-list`: ASA ACL permit 접두부와 양의 `(hitcnt=10)` 시안, `(hitcnt=0)` 회색.

화면에 실제 deny ACL 행과 `TCP PAT` 행은 존재하지 않아 그 양성 사례는 통합 화면에서 아직 확인하지 않았습니다. 별도 Extended 목록에서 확인한 `TCP PAT`와 `TCP conn`/`UDP conn` 오탐 수정 결과는 통합 목록 검증으로 확대하지 않습니다. ASA 화면 검증 범위는 `asa/ASA-SecureCRT-v1-README.md`에 기록합니다.
