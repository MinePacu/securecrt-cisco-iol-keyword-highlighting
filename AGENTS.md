...
이 파일은 이 프로젝트를 수정하는 AI 에이전트를 위한 지침이다. 2026-09-17 ASA·터널 통합 자동 검증과 기존 사용자 화면 검증까지 반영했다. 과거 대화 없이 시작하더라도 먼저 이 파일과 `docs/NAT-Highlighting-Status.md`, `docs/ASA-Highlighting-Status.md`, `docs/Tunneling-Highlighting-Status.md`를 읽는다. 이후 새로운 사용자 요청이나 실제 검증 결과가 있으면 그 근거와 함께 갱신한다.
...
- 현재 두 운영 목록은 515개 행, 선언값 `00000203`이다. 행 수를 바꾸면 선언값·V2/V3 대응·테스트·README를 함께 갱신한다. 이 수치는 영구 제한이 아니라 현재 기준이다.
- 통합 V3를 재설치한 뒤 사용자 화면에서 NAT, ACL, 일부 라우팅 출력이 정상인 것을 확인했다. 이미 확인한 사례를 다시 처음부터 진단하지 않는다.
- GRE·DMVPN/NHRP·IKE/IPsec 규칙은 자동 검사를 통과했다. 사용자 제공 `show crypto ipsec sa` 화면을 근거로 관련 행과 crypto-map tag를 보강한 뒤, Cisco 공식 명령 참조의 실제 출력에 근거해 `show crypto isakmp sa` 및 `show crypto map` 규칙을 추가했다. ISAKMP의 `QM_IDLE`은 초록, `MM_*`/`AG_*` 협상 상태는 주황, `STDBY`는 중립 은색이다. crypto map은 이름·프로토콜·Peer·ACL·SA lifetime·PFS·transform set·RRI·적용 인터페이스를 강조한다. 후속 사용자 화면에서 ASA 요약 인터페이스 전체 행 규칙이 IOS `show ip interface brief`에도 적용되어 정상 행 전체가 초록, administratively-down 행 전체가 회색이 되는 충돌을 확인했다. 이를 기존 토큰 색상으로 보호하는 `IOS_SHOW_IP_INTERFACE_BRIEF` 6개 규칙을 ASA 블록 앞에 추가하여 515행 V3를 `-SkipUpdate`로 재설치했다. 설치본은 선언값 `00000203`, SHA256 `ABC0AB99A3F84050430CCD02CF4E0169924852B4CE1BC467FBBE1E378D7F1B66`으로 소스와 일치하며 변경 후 화면 검증은 남아 있다.
- `dist/upstream-main`의 ASA Extended 63개 정규식은 `ASA_FIREWALL_OPERATIONAL_STATES`로 두 운영 목록에 그대로 병합했다. 이 ASA 규칙은 과거 432행 통합 목록에서 일부 실제 화면 검증 증거가 있다. 현재 소스와 설치본은 모두 515행이다. 직전 494행 설치본은 `PNET-Cisco-Dark-V3.ini.bak-20260918-110437354`으로 보존됐다.
- 515행 인터페이스 요약 보호까지 포함한 상태는 2026-09-18 정식 `v1.2.1` 릴리즈로 `main`에 게시했다. 이후 변경은 다시 `Unreleased`에 기록한다.
...
| IOS `show ip interface brief` | 494행 설치본에서 ASA 전체 행 규칙이 정상 행 전체를 초록, administratively-down 행 전체를 회색으로 덮는 충돌 확인; 515행 보호 규칙을 재설치했으며 변경 후 화면 검증은 남아 있음 |
...
4. `BGP_SHOW_IP`: 기존 BGP 상세 출력 우선순위를 유지.
5. `IOS_SHOW_IP_INTERFACE_BRIEF`: IOS와 ASA가 공유하는 요약 행 형식에서 기존 인터페이스·주소·상태 토큰 색상을 ASA 전체 행 규칙보다 먼저 보호.
6. `ASA_FIREWALL_OPERATIONAL_STATES`: ASA Extended 63개 정규식을 그대로 유지하고 터널 및 범용 상태 규칙보다 먼저 평가.
7. `TUNNEL_GRE_INTERFACE` → `DMVPN_NHRP` → `IKE_IPSEC_STATUS`: 범용 `up/down/disabled/active` 규칙보다 먼저 평가.
8. 나머지 기존 블록. 일반 IPv4 규칙을 NAT 앞에 무작정 올리지 않는다.
...
ASA 블록은 보존된 `dist/upstream-main/asa/ASA-SecureCRT-v1-Extended.ini`의 default-color fallback을 제외한 63개 정규식과 패턴·색상·순서가 같아야 한다. 공통 형식인 IPsec `#pkts encaps/decaps`, nonzero `#send/#recv errors`는 ASA 블록이 일반 터널 블록보다 먼저 적용되어 ASA 정보색/주의색을 사용한다. 단, ASA와 IOS의 brief 인터페이스 행은 줄 단위로 구분할 수 없으므로 앞선 `IOS_SHOW_IP_INTERFACE_BRIEF`가 두 플랫폼에 동일한 토큰식 색상을 적용한다.
...
- `InterfaceBriefHighlights.Tests.ps1`은 V2/V3에서 IOS/ASA 공통 brief 행의 인터페이스·주소·기본 필드·up/down 토큰이 ASA 전체 행 규칙보다 먼저 기존 색상을 받는지와 대표 불완전 행 오탐을 확인한다. 자동 검사는 통과했지만 변경 후 SecureCRT 화면 검증은 남아 있다.
...
새 NAT 화면 결과를 얻으면 `docs/NAT-Highlighting-Status.md`, ASA는 `docs/ASA-Highlighting-Status.md`, 터널은 `docs/Tunneling-Highlighting-Status.md`와 이 파일 및 README를 함께 갱신한다. 과거 Probe 문서의 “대기/가설”과 최신 확인 결과를 구분하고, 확인된 통합 성공 사례를 다시 미확인 상태로 되돌리지 않는다.
